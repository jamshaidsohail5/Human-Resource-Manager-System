...
/**
 * Created by jamsh on 3/18/2017.
 */
@WebServlet(name = "CoverPhotoUpload" , urlPatterns = {"/coverphotoservlet"})
@MultipartConfig
public class CoverPhotoUpload extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (isMultipart) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (!item.isFormField()) {
                        try {
                            ...
                            in.setCoverPhoto(fileName);
                            if (!"".equals(value)) {
                                String root = getServletContext().getRealPath("/");
                                File path = new File(root + "/coverphotos");
                                if (!path.exists()) {
                                    path.mkdirs();
                                }
            ...
            if(type.compareTo("Applicant")==0 ) {
                String result = db.uploadcover(in);
                result = "Cover Photo loaded successfully";
